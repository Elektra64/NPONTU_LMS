...
let courses = [
    {
        ...
        description:
            "Learn full-stack web development with modern technologies",
        duration_weeks: 12,
        hours_per_week: 8,
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    modulesContainer.addEventListener("click", function (e) {
        // Remove module
        if (
            e.target.classList.contains("remove-module") ||
            e.target.closest(".remove-module")
        ) {
        ...
        // Add question
            ...
            e.target.classList.contains("add-question") ||
        ...
        // Remove question
            ...
            e.target.classList.contains("remove-question") ||
...
function addModule() {
    const moduleCount =
        modulesContainer.querySelectorAll(".module-item").length + 1;
    ...
    courseList.innerHTML = courses
        ...
        <div class="grid grid-cols-12 gap-4 p-4 border-b border-gray-200 hover:bg-gray-50 transition-colors">
            <div class="col-span-5 flex items-center">
                <div class="w-16 h-12 ${getCategoryBgClass(
                    course.category
                )} rounded-md flex items-center justify-center mr-4">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 ${getCategoryTextClass(
                        course.category
            ...
            <div class="col-span-2 flex items-center">
                <span class="${getCategoryBadgeClasses(
                    course.category
                )} px-2 py-1 rounded-full text-xs">${course.category}</span>
            </div>
            ...
            <div class="col-span-1 flex items-center">
                <span class="${getStatusBadgeClasses(
                    course.status
                )} px-2 py-1 rounded-full text-xs">${course.status}</span>
            </div>
            ...
            <div class="col-span-1 flex items-center space-x-2">
                ...
                <button class="text-red-600 hover:text-red-800" onclick="deleteCourse('${
                    course.id
                }')">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" viewBox="0 0 20 20" fill="currentColor">
...
function createNewCourse() {
    ...
    moduleElements.forEach((moduleEl) => {
        const moduleTitle = moduleEl.querySelector(
            'input[name*="title"]'
        ).value;
        const durationDays = moduleEl.querySelector(
            'input[name*="duration_days"]'
        ).value;
        const moduleDescription = moduleEl.querySelector(
            'textarea[name*="description"]'
        ).value;
        const quizWeight = moduleEl.querySelector(
            'input[name*="quiz_weight"]'
        ).value;
        ...
        questionElements.forEach((questionEl) => {
            ...
            const options = Array.from(
                questionEl.querySelectorAll('input[name*="options"]')
            ).map((opt) => opt.value);
            const correctAnswer =
                questionEl.querySelector('input[name*="correct"]:checked')
                    ?.value || "0";
    ...
    alert(
        `Course "${title}" created successfully with ${modules.length} modules!`
    );
}
...
function editCourse(courseId) {
    ...
    document.querySelector('input[name="duration_weeks"]').value =
        duration_weeks;
    document.querySelector('input[name="hours_per_week"]').value =
        hours_per_week;
    document.querySelector('input[name="video_url"]').value = video_url;
    document.querySelector('input[name="final_exam_weight"]').value =
        final_exam.weight;
    document.querySelector('input[name="passing_score"]').value =
    ...
    modules.forEach((module, moduleIndex) => {
        ...
        moduleItem.querySelector('input[name*="duration_days"]').value =
            module.duration_days;
        moduleItem.querySelector('textarea[name*="description"]').value =
            module.description;
        moduleItem.querySelector('input[name*="quiz_weight"]').value =
            module.quiz_weight;
        ...
        if (module.questions && module.questions.length > 0) {
            module.questions.forEach((question, questionIndex) => {
                ...
                // Fill question data
                questionItem.querySelector('input[name*="text"]').value =
                    question.text;

                // Fill options
                const optionInputs = questionItem.querySelectorAll(
                    'input[name*="options"]'
                );
                ...
                if (
                    question.correct >= 0 &&
                    question.correct < optionInputs.length
                ) {
                    const correctInput = questionItem.querySelector(
                        `input[name*="correct"][value="${question.correct}"]`
                    );
                    if (correctInput) {
